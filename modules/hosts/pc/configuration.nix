  ...
  flake.nixosModules.pcConfig = { pkgs, lib, config,  ... }: {
    ...
    services.pipewire = {
      ...
      extraConfig.pipewire = {
        ...
        # This was causing some audio crackling
        # "92-low-latency" = {
        #   "context.properties" = {
    ...
    systemd.user.services.start-clipping = {
      ...
      serviceConfig = {
        Type = "simple";
        ExecStart = "${pkgs.gpu-screen-recorder}/bin/gpu-screen-recorder -w \"DP-2;width=1920;height=1080;halign=center;valign=center\" -c mkv -s 1920x1080 -f 60 -cursor yes -a \"alsa_input.usb-HP__Inc_HyperX_Cloud_III_Wireless_0000000000000000-00.mono-fallback|alsa_output.usb-HP__Inc_HyperX_Cloud_III_Wireless_0000000000000000-00.analog-stereo.monitor\" -k hevc_10bit -bm cbr -q 30000 -cr full -tune quality -r 300 -replay-storage ram -df yes -restart-replay-on-save no -o /home/hadif/Videos/Clips";
      };
    };
